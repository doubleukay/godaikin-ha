...
from .mqtt import BRIDGE_AVAILABILITY_TOPIC, DISCOVERY_PREFIX, MQTT_PREFIX
from .types import *
...
class Controller:
    ...
    async def publish_discovery(self):
        ...
        for aircond in airconds:
            self.set_mqtt_topics(aircond)
            await self.publish_aircond_discovery(aircond)
            await self.publish_sensor_discovery(aircond)

    def set_mqtt_topics(self, aircond: Aircond):
        # Cache these MQTT topics to avoid reevaluating them multiple times
        topics = AircondMqttTopics.from_aircond(aircond)
        self.mqtt_topics_by_unique_id[aircond.unique_id] = topics

    async def publish_aircond_discovery(self, aircond: Aircond):
        ...
        await self.mqtt_publish(topics.discovery, payload, qos=1, retain=True)

    async def publish_sensor_discovery(self, aircond: Aircond):
        topics = self.mqtt_topics_by_unique_id[aircond.unique_id]
        ...
        for (
            ...
            sensor_discovery_topic = f"{DISCOVERY_PREFIX}/sensor/{aircond.unique_id}/{sensor_norm_name}/config"
            payload = {
                "name": sensor_name,
            ...
            await self.mqtt_publish(sensor_discovery_topic, payload, qos=1, retain=True)
